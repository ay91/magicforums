...
(function() {
  var postsChannelFunctions;

  postsChannelFunctions = function() {
    var checkMe, createComment, destroyComment, updateComment;
    checkMe = function(comment_id) {
      if ($('meta[name=admin]').length < 1) {
        return $(".comment[data-id=" + comment_id + "] .control-panel").remove();
      }
    };
    createComment = function(data) {
      ...
      if ($('.comment-container').data().id === data.post.id) {
        ...
        return checkMe(data.comment.id);
      }
    };
    ...
    if ($('.comment-container').length > 0) {
      return App.posts_channel = App.cable.subscriptions.create({
...
(function() {
  var votesChannelFunctions;

  votesChannelFunctions = function() {
    if ($('.comment-container').length > 0) {
      return App.votes_channel = App.cable.subscriptions.create({
        channel: "VotesChannel"
      }, {
        connected: function() {},
        disconnected: function() {},
        received: function(data) {
          return $(".comment[data-id=" + data.comment_id + "] .comment-total-votes").html(data.value);
        }
      });
    }
  };
